...
import ExerciseV2Row from '~/components/cms/exercisesV2/ExerciseV2Row.vue'
...
const filteredExercises = computed(() => {
  ...
  return exercisesStore.exercises.filter((ex) => {
    ...
    if (equipmentFilter.value && !ex.equipmentOptions.includes(equipmentFilter.value)) return false
...
<template>
  <div>
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
      <h1 class="text-xl font-bold text-forge-text">Ejercicios v2</h1>
      <div class="flex flex-wrap gap-2">
        ...
        >
          ...
          <option v-for="opt in EQUIPMENT_OPTIONS" :key="opt" :value="opt">{{ EQUIPMENT_LABELS[opt] }}</option>
        </select>
        <select
