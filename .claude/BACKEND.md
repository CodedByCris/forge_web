...
- **Storage usa un mecanismo de admin distinto a Firestore (2026-08-10, corrección de bug real en producción).** Las 4 rutas de Storage que sube el CMS (`exercises`, `whats_new`, `getting_started`, `shop_items`) comprobaban `isAdmin` con `firestore.get()` (llamada cruzada Storage→Firestore) — esa llamada **nunca funcionó en producción** en este proyecto (sí en el emulador local, nunca en real), así que **todas las subidas de archivo desde el CMS (imagen de portada, Lottie, audio) fallaban siempre con `storage/unauthorized`**, aunque guardar los campos de texto de esos mismos formularios (que solo tocan Firestore) funcionaba con normalidad — de ahí que pasara desapercibido. Corregido usando un custom claim de Firebase Auth (`admin: true`) en vez de `firestore.get()`. **Cualquier admin nuevo del CMS necesita AMBOS**: `users/{uid}.isAdmin = true` (gate de login del CMS + reglas de Firestore) **y** el custom claim `admin: true` (`admin.auth().setCustomUserClaims(uid, {admin:true})`, solo Admin SDK) para que las subidas a Storage funcionen — y debe cerrar sesión/volver a entrar tras fijarse el claim.
- **Bug relacionado, mismo fix:** borrar un archivo ya subido (imagen/Lottie/audio) también fallaba siempre — las reglas de Storage compartían un único `allow write` con condiciones sobre `request.resource.size`/`contentType`, que es `null` en un delete. Separado en reglas de `create, update` vs `delete`.
